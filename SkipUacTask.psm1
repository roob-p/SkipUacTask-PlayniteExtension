...
$gamesToProcess = $gamesel | Where-Object {
    -not $_.gameactions -or $_.gameactions.Count -eq 0 -or $_.gameactions[0].name -notcontains "SkipUacTask"
}


if ($gamesel.count -le 1) {
if ($gamesToProcess.Count -eq 0) {
	$playniteApi.Dialogs.ShowMessage("$($gamesel): SkipUacTask is already the 1st entry in gameactions.")
    return
}
}
else{
	if ($gamesToProcess.Count -eq 0) {
    $playniteApi.Dialogs.ShowMessage("All selected games already have SkipUacTask as 1st action. Nothing to do.")
    return
}
}
...
if(!$game.gameactions -and !$game.source){$playniteApi.Dialogs.ShowMessage("$($game.name): There aren't any game actions.")
$global:ga=$True
#$global:go=$False
allscripts-alt
...
if ($baext -ne ".lnk" -and $baext -ne ".exe" -and $baext -ne ".bat" -and $baext -ne ".cmd" -and !$game.source){ $playniteapi.dialogs.ShowMessage("$($game.name): $($baext) not supported!")
$global:ga=$True#
	}
}
}
...
if ($game.gameactions[0].name -notcontains "SkipUacTask"){
	...
	if ($baext -ne ".lnk" -and $baext -ne ".exe" -and $baext -ne ".bat" -and $baext -ne ".cmd" -and !$game.source){  #new
		$global:ga=$True  #new
		#$global:commandsx+="#"
		allscripts-alt
		}                 #new
...
if ((($game.gameactions[0].type -eq "File") -or ($game.gameactions[0].type -eq "URL")) -and ($baext -ne ".lnk" -and $baext -ne ".exe" -and $baext -ne ".bat" -and $baext -ne ".cmd") -and !$game.source) { #new
	$global:ga=$True #new
	#allscripts		 #new
...
else {$playniteapi.dialogs.ShowMessage("$($game.name): SkipUacTask is already the 1st entry in gameactions.")
$global:ga=$True
#$playniteApi.Dialogs.ShowMessage("already ga")
#$global:go=$False
#$commands="###"
#$global:commandsx+="###"
allscripts}
...
if(!$game.gameactions){
	$global:go=$True
	#$playniteApi.Dialogs.ShowMessage("4_noGA")
	create4_noGA
	  }#endif
    }#end elseif
...
##crea:
function create()                                       #CREATE+BUILD(included) FOR EMULATOR and EXE WITHOUT SOURCE
{
		param(
		$getGameMenuItemsArgs
	)
...
if ($game.gameactions[0].type -eq "Emulator") {								#EMULATOR
...
if (($game.gameactions[0].type -eq "File") -or ($game.gameactions[0].type -eq "URL")){						#FILE or URL
...
function create2()                      #GAME SOURCE UNKOWN (source -ne Steam, Epic, etc)
{
		param(
		$getGameMenuItemsArgs
	)
...
function create3()        #EXTENSION UNKOWN (#ext -ne bat, lnk, cmd, exe)   #yes source, type file or url 
{
		param(
		$getGameMenuItemsArgs
	)
...
function create4_noGA()        #NO GAMEACTIONS (credo relativo a SOURCE)
{
		param(
		$getGameMenuItemsArgs
	)
...
$global:commandsx+=$commands + "`r`n"


if (($global:ga -eq $False) -and ($game.source) )  {
if (-not $global:killtype.ContainsValue($game.source.name)) {
    #$global:killType[$game.source.name] = $true
	$global:killtype[$global:ks]=$game.source.name
	$global:killList[$global:ks]=$commandskill
    #$global:commandsx += $commandskill + "`r`n"
}
}else{
	#$global:killtype[$global:ks]="##"
	#$global:killList[$global:ks]="##"
}


#if ($global:ks -eq $gamesel.count) {
if (($global:ks -eq $gamesel.count)-and ($global:go -eq $True)) {
$global:commandsx += "`r`n"
$global:commandsx += "`r`n"
$global:commandsx += "# ---CLIENT KILL---"
$global:commandsx += "`r`n"


foreach ($source in $global:killtype.Keys) { #OK
    $global:commandsx += $global:killList[$source] + "`r`n"
}


$global:commandsx+="start-sleep -s 2"



$tempx = "$env:TEMP\RunAllScripts.ps1" 

$scriptContent = $global:commandsx
$scriptContent | Set-Content -Encoding UTF8 -Path $tempx -Force


Start-Process "powershell" -Verb RunAs -ArgumentList "-ExecutionPolicy Bypass -File `"$tempx`""
